...
URI = f'https://www.pexels.com/search/videos/{argv[1]}/'
...
# This option when set to False will display the chrome window
# True hides it
...
scroll_pause_time = 1 # You can set your own pause time. If your PC is a bit slow use 1 sec
...
list_links = driver.find_elements(By.CSS_SELECTOR, 'a.Link_link__mTUkz')
...
for i in list_links:
    ...
    # https://www.pexels.com/video/3129671/download/
    downloadlink = f"https://www.pexels.com/video/{fileurl[-1]}/download/"
    with open(f"{filename[-1]}.mp4", "wb") as file:
        # get request
            response = requests.get(downloadlink)
            # write to file
            print(f"\u001b[33mDownloading {filename[-1]}.mp4")
